...
public class ProgramUser {
  ...
  public int getCurrentEmployeeId() {
    return employeeIdOfCurrentUser;
  }
   ...
   *                      needed for the Log-in verification process or if there
   *                      is no
   *                      registered Employee. The program user goes through the
   *                      Admin
   *                      Registration Process before going through a Log-in
   *                      Verification
   *                      process.
   * @param isCLIVersion  Boolean variable to determin whether to use
   *                      the CLI Version of the program. If false, the GUI
   *                      version of the
   *                      program is used.
   * 
   */
  ...
  private void determineWhetherUserIsAdminOrNot(ResultSet employeeData, Employees employeeTable) {
    ...
    try {
      ...
      employeeIdOfCurrentUser = Integer.parseInt(
          employeeData.getString(employeeTable.employeeidColumn));

      String username = employeeData.getString(employeeTable.firstnameColumn) + " " +
          employeeData.getString(employeeTable.lastnameColumn);

      String message = (isAdminEmployee) ? "User " + username + " is an Admin Employee"
